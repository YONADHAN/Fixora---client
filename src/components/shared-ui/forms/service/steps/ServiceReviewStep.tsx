  ...
  return (
    <div className='max-w-4xl mx-auto bg-white border rounded-2xl shadow-sm p-6 space-y-8'>
      ...
      <div className='border rounded-xl p-4 space-y-2'>
        ...
        <p className='text-sm'>
          <b>Sub Category ID:</b> {values.subServiceCategoryId || '—'}
        </p>
      </div>
      ...
      <div className='border rounded-xl p-4 space-y-2'>
        ...
        <p className='text-sm'>
          <b>Price per Slot:</b> ₹{values.pricing?.pricePerSlot ?? 0}
        </p>

        <p className='text-sm'>
          <b>Advance Amount:</b> ₹{values.pricing?.advanceAmountPerSlot ?? 0}
        </p>
      </div>
      ...
      <div className='border rounded-xl p-4 space-y-3'>
        ...
        <p className='text-sm'>
          <b>Slot Duration:</b> {values.schedule?.slotDurationMinutes ?? 0} min
        </p>

        <p className='text-sm'>
          <b>Recurrence:</b> {values.schedule?.recurrenceType || '—'}
        </p>

        {/* ✅ DAILY WINDOWS */}
        <div>
          <p className='font-medium text-sm mb-1'>Daily Working Windows</p>
          ...
          {values.schedule?.dailyWorkingWindows?.length > 0 ? (
            ...
            <p className='text-sm text-gray-500'>No windows added</p>
          )}
        </div>
        ...
        {values.schedule &&
          ...
          values.schedule?.weeklyWorkingDays?.length > 0 && (
            <p className='text-sm'>
              <b>Weekly Days:</b> {values.schedule.weeklyWorkingDays.join(', ')}
            </p>
          )}
      ...
      {values.serviceVariants && values.serviceVariants?.length > 0 && (
        <div className='border rounded-xl p-4 space-y-2'>
          <h3 className='font-medium text-gray-800 mb-2'>Service Variants</h3>
      ...
      {/* ✅ IMAGE PREVIEW */}
      <div className='border rounded-xl p-4 space-y-3'>
        <h3 className='font-medium text-gray-800'>Main Service Image</h3>
