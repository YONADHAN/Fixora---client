...
import {
  useVendorRequests,
  useChangeVendorVerificationStatus,
...
const VendorVerificationPage = () => {
    ...
    changeStatus(
      {
        ...
        description: `Admin approved vendor ${selectedVendor.name}`,
      },
      {
        onSuccess: async () => {
          toast.success(` Vendor ${selectedVendor.name} approved successfully.`)
          setShowDialog(false)
          await refetch()
        },
        onError: (error: any) => {
  ...
  const handleReject = () => {
    if (!selectedVendor) return
    if (!rejectionReason.trim()) {
      toast.warning('Please enter a rejection reason before rejecting.')
      return
    }
    ...
    changeStatus(
      ...
      {
        onSuccess: async () => {
          toast.success(` Vendor ${selectedVendor.name} rejected successfully.`)
          setShowDialog(false)
          await refetch()
        },
        onError: (error) => {
