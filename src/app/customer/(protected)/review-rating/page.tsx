...
export default function ReviewRatingPage() {
  ...
  const [selectedService, setSelectedService] =
    useState<IBookedServiceReviewData | null>(null)
  const [isModalOpen, setIsModalOpen] = useState(false)

  const { bookedServices, isServicesLoading, servicesError } =
    useReviewRating(page)
  ...
  return (
    <div className='container mx-auto py-8 space-y-8'>
      ...
      <div className='flex flex-col gap-2'>
        ...
        <p className='text-muted-foreground'>
          Share your feedback on services you’ve used. Your reviews help others
      ...
      {services.length === 0 ? (
        <div className='flex flex-col items-center justify-center py-16 text-center border rounded-lg bg-muted/20'>
          ...
          <p className='text-muted-foreground'>
            You can review services only after the booking is completed.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
            {services.map((service: IBookedServiceReviewData) => (
              <ReviewCard
                key={service.ratingReviewId ?? service.serviceRef}
                data={service}
                onEdit={handleEdit}
              />
            ))}
